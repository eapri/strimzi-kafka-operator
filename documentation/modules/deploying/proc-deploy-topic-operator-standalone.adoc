...
You can set the value to `plain`, `scram-sha-256`, `scram-sha-512`, or `aws-msk-iam`.
...
. Look at the following example, if you want to connect to an AWS MSK with IAM authentication.
+
.Example AWS MSK IAM AUTH configuration (without TLS AUTH)
[source,shell,subs=+quotes]
----
# ....
env:
